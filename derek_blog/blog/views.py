...
def pagination_data(paginator, page):
    ...
    #当前页右边连续的页码号
    right =[] # 4，“5，6，7”
    left_has_more = False #标示第一页页码后是否需要显示省略号，1，....,,9,10
    right_has_more = False #标示最后一页页码前是否需要显示省略号,1,2,3,....10
    #标示是否需要显示第1页的页码号
    #因为如果当前页左边的连续页码号中已经含有第1页的页码号，此时就无需再显示第1页的页码号
    #其他情况下第一页的页码是始终需要显示的
    #初始值为False
    first = False  #1,2,3,...,10
    #标示是否需要显示最后一页的页码号
    #需要此遍历的理由和上面相同
    last = False # 1,....8,9,10
    #获得用户当前请求的页码号
    ...
    if page_number == 1:
        ...
        right = page_range[page_number:page_number+4] #拿4个元素
        if right[-1] < total_pages -1: # 1,"2,3,4,5",....20
            right_has_more = True #
        if right[-1] < total_pages: #显示最后一页的页码数
            last = True
        ...
        left = page_range[(page_number -4 ) if (page_number-4) > 0 else 0:page_number-1] # 1,....."17,18,19,"20
        # 如果最左边的页码号比第 2 页页码号还大，
        # 说明最左边的页码号和第 1 页的页码号之间还有其它页码，因此需要显示省略号，通过 left_has_more 来指示。
        if left[0] > 2 : #1,....17,18,19,20
            left_has_more = True
        ...
        left = page_range[(page_number-4) if (page_number -4 ) >0 else 0: page_number -1] #  1,..7,8,9,10,11,12,,...20
...
def permission_denied(request,exception):
    '''403'''
    return render(request, 'blog/403.html', locals(), status=403)


def page_not_found(request,exception):
    '''404'''
    return render(request, 'blog/404.html', locals(), status=404)
...
def category(request,category_id):
    entries = models.Entry.objects.all().filter(category__id=category_id)
    page = request.GET.get('page', 1)
    entry_list,paginator = make_paginator(entries, page)
    page_data = pagination_data(paginator, page)
    return render(request, 'blog/index.html', locals())
...
def tag(request, tag_id):
    entries = models.Entry.objects.all().filter(tags__id=tag_id)
    page = request.GET.get('page', 1)
    entry_list, paginator = make_paginator(entries, page)
    page_data = pagination_data(paginator, page)
    return render(request, 'blog/index.html', locals())
